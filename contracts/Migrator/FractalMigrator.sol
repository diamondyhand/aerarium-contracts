...
contract TokenSwap {
    ...
    function swap(uint256[] memory _tokenIds) public {
        ...
        require(numNewTokens >= _tokenIds.length, "Not enough new tokens in contract");
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 newTokenId = IERC721Enumerable(address(newToken)).tokenOfOwnerByIndex(address(this), i);
    ...
    function depositNewTokens(uint256[] memory _tokenIds) public {
        require(msg.sender == owner, "Only the contract owner can deposit new tokens.");
        for (uint i = 0; i < _tokenIds.length; i++) {
